...
import com.pdd.video.Crawler.aiqiyiMusicCrawlerLink;
import com.pdd.video.Crawler.aiqiyiMusicCrawlerTag;
...
public class Music_Main {
	public static void main(String[] args) {
		try {
			List<urlBase> TVUrls=aiqiyiMusicCrawlerLink.getLinks();
			for (urlBase urlBase : TVUrls) {
				pageBase page=aiqiyiMusicCrawlerTag.getPage(urlBase);
				...
				try {
					...
					if(status>0){
						System.out.println("新增数据成功!"+page.getVid());
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
